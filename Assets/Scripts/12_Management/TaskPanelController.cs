...
namespace SpaceGame.UI
{
    ...
    public class TaskPanelController : MonoBehaviour
    {
        ...
        // interner Index: Dropoff-Eintr�ge
        readonly List<Transform> _dropoffTargets = new();

        void OnEnable()
        {
            // Dropdowns initialisieren, wenn Panel ge�ffnet wird
            BuildModeDropdown();
            BuildRegionDropdown();
        ...
        void BuildRegionDropdown()
        {
            ...
            dropdownRegion.value = 1; // h�ufigster Fall
        }
        ...
        void BuildDropoffDropdown()
        {
            ...
            if (options != null && options.Count > 0)
            {
                foreach (var (id, label) in options)
                {
                    ...
                    // L�sung: wir legen ein Dummy-Transform NICHT an, sondern speichern die ID tempor�r.
                    // -> Erg�nze unten: _dropoffHubIds parallel tracken.
                }
            }
            else
            {
