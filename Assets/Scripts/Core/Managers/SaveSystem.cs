...
public class SaveSystem : MonoBehaviour
{
    ...
    [Header("Allgemein")]
    [Tooltip("Version des Save-Formats � bei �nderungen erh�hen und ggf. Migration implementieren.")]
    ...
    /// <summary>L�dt den Spielstand aus dem Slot. Erwartet, dass die Spielszene aktiv ist.</summary>
    public async UniTask<bool> LoadAsync(string slotId)
    {
        ...
        if (save.version != saveVersion)
            Debug.LogWarning($"[SaveSystem] Save-Version {save.version} != erwartete {saveVersion}. Migration n�tig?");
    ...
    private List<EntitySaveData> CaptureEntities()
    {
        ...
        foreach (var e in registry.All)
        {
            ...
            catch (Exception ex) { Debug.LogError($"[SaveSystem] Capture fehlgeschlagen f�r Entity: {ex}"); }
        }
        return list;
    }
    ...
    private async UniTask RespawnFromSave(SaveGame save)
    {
        ...
        foreach (var data in save.entities)
        {
            ...
            {
                Debug.LogError($"[SaveSystem] Respawn fehlgeschlagen f�r {data?.TypeId}: {ex}");
            }
        }
    }
    ...
    private void RestoreHUD(HUDLayoutSaveData data)
    {
        ...
        foreach (var p in panels)
        {
            ...
            try
            {
                ...
                // Reihenfolge: Gr��e/Pivot vor Position, damit Layout korrekt gerechnet wird
                rt.pivot = s.pivot;
