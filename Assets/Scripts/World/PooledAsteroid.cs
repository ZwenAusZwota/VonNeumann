...
/// <summary>
/// Komponente f�r gepoolte/verwaltete Asteroiden mit Low/High-LOD-Visuals.
/// Berechnet nach LOD-Erzeugung einen sicheren Nav-Radius �ber alle Kinder-Renderer/Collider.
/// </summary>
public class PooledAsteroid : MonoBehaviour
{
    ...
    [Tooltip("Zieltransform (z. B. Sonde) f�r den Distanz-Swap")]
    public Transform proximityTarget;

    [Tooltip("Abstand (Unity-Units), ab dem High angezeigt wird")]
    public float swapDistanceUU = 150f;

    [Tooltip("Hysterese (Unity-Units) � zur�ck zu Low, wenn Distanz > swap + hysteresis")]
    ...
    /// <summary>Erzeugt/konfiguriert die Visuals. Kann mehrfach f�r Reuse gerufen werden.</summary>
    ...
    /// <summary>Wird vom Belt f�r Culling aufgerufen.</summary>
    ...
    /// <summary>Rebuild von Renderer/Collider-Listen nach LOD-�nderungen.</summary>
    ...
    private void EnsureHighInstance()
    {
        ...
        // >>> NEU: Nachladen von High kann Gr��e �ndern -> Radius neu bestimmen
        var mine = GetComponent<MineableAsteroid>();
        if (mine != null) mine.RecalculateNavSphereRadiusFromHierarchy(1.1f);
    }

    /// <summary>F�r Pool-Reuse oder manuelles Zur�cksetzen.</summary>
    public void ResetAsteroid()
    {
        // Sichtbarkeit zur�cksetzen
        SetVisible(true);
        ...
        // R�cksetzen von Transform-Basiswerten
    ...
    // Hilfsfunktionen (Tagging/Collider f�r LOD-Kinder)
    ...
    {
        if (go == null) return;
        // Wenn im gesamten Hierarchiebaum kein Collider existiert, f�ge einen SphereCollider am Root hinzu.
        if (go.GetComponentInChildren<Collider>(true) == null)
        {
            var sc = go.AddComponent<SphereCollider>();
            sc.isTrigger = false; // f�r Scan/Ann�herung ausreichend
